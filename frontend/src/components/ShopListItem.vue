...
function setInCart() {
	...
	itemsStore.setItemStatus(item.item_id, ItemStatus.inCart);
}
function setBuy() {
	const item = props.item.value;
	itemsStore.setItemStatus(item.item_id, ItemStatus.buy);
}
	...
	<div class="p-2 border-b relative grid item-grid" :class="[in_cart ?'bg-green-50' : 'bg-white']">
		<div class="min-w-0" @click="expanded = !expanded">
			<h2 class="overflow-hidden overflow-ellipsis text-xl" 
				:class="[in_cart ? ['text-green-800/50', 'font-normal'] : ['text-black', 'font-bold']]">
				{{ item.value.name }}</h2>
		...
		<div class="flex justify-center items-center" >
			<button v-if="in_cart" @click.stop.prevent="setBuy">
			...
			<button v-else @click.stop.prevent="setInCart">
